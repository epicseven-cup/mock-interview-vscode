...
5. The AI interviewer responds in the chat panel. Ask follow-up questions or keep coding and review again.
...
| **Add Files to Context** | Manually add files or folders to the interview context for custom setups. |
...
**Mode bar**
- **Hint / Harsh / Follow-up** pills — click to switch interviewer personality. Resets conversation.
- **Model button** (right side) — shows the active model (`claude: sonnet-4-5`, `ollama: llama3`, `copilot`). Click to open the model picker for the current provider.

**Message area** — conversation history between you and the interviewer.

**Bottom bar**
- **Ready for Review** button — triggers a full review of your current solution files.
- **Text input** — chat with the interviewer. `Enter` to send, `Shift+Enter` for a new line.
...
- **Auto-detect on open** — if a `working-mock-interview-*/` directory already exists in your workspace, the session resumes automatically (context loaded, no AI triggered). If multiple sessions exist, you pick one.
- **New session** — if no working directory exists but a problem template is found, you're prompted to start. Clicking Start copies the template files into a new working directory and triggers the opening AI message.
- **Resume vs. Start Fresh** — clicking Start Interview on a problem you already have a working directory for asks you to choose. Start Fresh deletes the working directory and recopies the template from scratch.
...
| `mockInterview.apiKey` | — | Anthropic API key for Claude. Set via **Setup Claude** command. |
...
- Workspace problems: `.mock-interviews/mock-interview-problems-{name}/` (takes priority)
- Working directory: `{workspace}/working-mock-interview-{name}/` (auto-created on start)
- The folder name suffix becomes the label — `mock-interview-problems-linked-list-cycle` → **linked list cycle**
